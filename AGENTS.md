...
- `normalize_rule`: Uses an LLM (default: GPT-4.1) to generate a minimal YARA rule equivalent to the original — removes regex conditions and replaces complex count expressions (e.g. `5 of ($a*)`) with explicit string lists. Downstream nodes operate on `normalized_rule`.
- `judge_rule`: Uses the LLM to verify the normalized rule is a valid, correct subset of the original; on a `failed` verdict the pipeline retries `normalize_rule` (up to 3 attempts) or routes to `fail_normalization` when exhausted.
